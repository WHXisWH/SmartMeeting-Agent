...
このプロジェクトは、対話形式に頼ることなく「会議の必要性の検知 → 日程提案 → 開催確定 → 関連資料の作成と配布 → フォローアップ」という一連のプロセスを自律的に完結させる、Google CloudとVertex AIを基盤としたエージェントです。

## 設計コア
- 目的: Gmail、Googleカレンダー、Googleドライブのシグナルから必要な会議を検出し、Vertex AIによる意思決定を経て、Google Workspace上でアジェンダ、議事録、ToDoリストなどの成果物を生成・配布します。
- アプローチ: イベント駆動型アーキテクチャ、冪等性の制御、そしてポリシーに基づいた重み付けスケジューリングを採用しています。データはすべてGoogle Cloudネイティブのサービスで完結します。
- 自律ループ: 「感知 → 解析 → 決定 → 実行 → 学習」のサイクルを自律的に繰り返します。システムにはセーフティガードと監視機能が組み込まれています。

## アーキテクチャ
- Cloud Run (Node.js / Express): 公開API、Webhook（Gmail Push / Calendar Watch）、管理・内部ジョブをホストします。アプリケーションはステートレスに設計され、状態はFirestoreで管理されます。
- Cloud Functions (decisionTool): 状況解析とポリシー評価を行います（BigQueryをデータソースとして利用）。
- Vertex AI: モデルによる生成や推論は、Vertex AI SDK経由でのみ実行します（Gemini APIの直接呼び出しは行いません）。
- Google Workspace API: Gmail, Calendar, Drive, Docs, Slides, Sheetsを統合し、実務作業を自動化します。
- イベント処理: Pub/Sub（Gmail Push通知）、Calendar Webhook、バックグラウンドキューを活用し、確実なイベント処理を実現します。
...
## イベントフロー
- 受信: Gmail Push通知（Pub/Sub経由）またはGoogleカレンダーの更新をWebhookで受信し、冪等キーを用いて重複処理を防止しつつ、Firestoreのキューにタスクを登録します。
...
- データ / イベント
  - Firestore（状態管理、冪等性キー、ワークキュー、提案情報）
...
- ロギング / セキュリティ / 可観測性
  ...
  - Cloud Trace統合（オプション）: webhook→task→API跨段跟踪
- ユーティリティ
...
- 多基準資料関連付け: n-gram解析、ドメイン相関、アクセス履歴、MIME優先度による智能検索
...
## Deployment (Cloud Run) / デプロイ（Cloud Run）
...
English (placeholders; set on Cloud Run)
...
## OAuth & First-time Setup / OAuth と初回セットアップ
...
- 議事録 & アクションアイテム
  - `curl -s -X POST "${RUN_URL}/api/agent/minutes/generate" -H 'Content-Type: application/json' -d '{"transcript":"<YOUR_TEXT>"}'`
  - 期待値: Docs/Slides/Sheet 作成、イベントにリンク追記、Google Tasks 作成、担当者にメール
- 利用状況 & 週報
  - 週次利用: `curl -s "${RUN_URL}/api/insights/usage-weekly?days=7"`
  - 週報生成: `curl -s -X POST "${RUN_URL}/api/reports/weekly" -H 'Content-Type: application/json' -d '{"days":7}'`
  - 最新週報: `curl -s "${RUN_URL}/api/reports/weekly/latest"`
...
日本語
- コンテナがヘルシーにならない（PORT）
  - 多くは必須環境変数の不足です。上記の一括設定をもう一度実行してください。
- ログ（Cloud Run リビジョン）
  - `gcloud logging read 'resource.type="cloud_run_revision" AND resource.labels.service_name="<SERVICE_NAME>"' --limit=200 --format='value(textPayload)' --project=<PROJECT_ID>`
## Judge Login Flow (Multi‑User)
- Any reviewer can sign in with their own Google account via the frontend “Google Login”.
...
```mermaid
graph TB
  U[Reviewer (Google Account)] -->|Google Login| FE[Vercel Frontend]
  FE -->|/auth/login| OA[(Google OAuth)]
